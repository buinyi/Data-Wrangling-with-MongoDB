...
def shape_element(element):
    ...
    if element.tag == "node" or element.tag == "way" :
        ...
        for item in element.iter("tag"):
            #print(item.attrib)
            if problemchars.match(item.attrib['k']) is None:
                if 'addr:' == item.attrib['k'][0:5]:
                    if ":" not in item.attrib['k'][5:]:
                        #print(item.attrib)
                        #print("This is address!")
                        node_address[item.attrib['k'][5:]]=item.attrib['v']
                elif ':' in item.attrib['k']:
                    #print("This is another attribute!")
                    node[item.attrib['k']]=item.attrib['v']
                else:
                    #print("This is also another attribute!")
                    node[item.attrib['k']]=item.attrib['v']
          #  else:
          #      print(item.attrib['v'],item.attrib['k'])
          #      print(problemchars.match(item.attrib['v']))
...
def test():
    ...
    data = process_map('D:/Udacity/IntroToMongoDB/example5.osm', True)
    #pprint.pprint(data)
